...
runner.on "spawn", (info) ->
  syslogh.syslog syslogh.NOTICE, "#{info.repo} #{info.id} spawn\r\n"
runner.on "stop", (info) ->
  syslogh.syslog syslogh.NOTICE, "#{info.repo} #{info.id} stop\r\n"
runner.on "restart", (info) ->
  syslogh.syslog syslogh.NOTICE, "#{info.repo} #{info.id} restart\r\n"
runner.on "exit", (code, signal, info) ->
  str = "#{info.repo} exited with code #{code}"
  str += " from signal #{signal}" if signal?
  syslogh.syslog syslogh.NOTICE, str+"\r\n"
runner.on "error", (err, info) ->
  syslogh.syslog syslogh.NOTICE, "#{info.repo} #{info.id} error - #{addNewline err.toString()}"
